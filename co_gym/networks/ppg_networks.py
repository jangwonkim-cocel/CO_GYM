...
class PPGCritic(BaseCritic):
    def __init__(self, obs_dim, hidden_dims, activation_fc_name):
        super(PPGCritic, self).__init__(activation_fc_name)
...
class PPGPolicy(BaseActor):
    def __init__(self, obs_dim, act_dim, hidden_dims, log_std_bound, activation_fc_name):
        ...
        self.aux_v_layer = nn.Linear(hidden_dims[-1], 1)
    ...
    def get_action(self, state, eval, worker_device):
        ...
        with torch.no_grad():
            if eval:
                _, log_prob, action, _ = self.sample(state)
            else:
                action, log_prob, _, _ = self.sample(state)
    ...
    def get_aux_value(self, state):
        x = self.to_tensor(state)
        x = self.activation_fc(self.input_layer(x))
        for i, hidden_layer in enumerate(self.hidden_layers):
            x = self.activation_fc(hidden_layer(x))
        aux_v = self.aux_v_layer(x)
        return aux_v
